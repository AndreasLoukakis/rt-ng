...
`<a [routerLink]="['/inbox']" routerLinkActive="pf-m-current" class="pf-c-nav__link">Inbox</a>`
...
-Notes: about ng-template, template binding and inline referenece
...
- Note: alternative ways of passing data to a component: router
...
There are several ways of passing data to a component, like input or an external service or application state. But we can also pass data via the router, which sounds appropriate for handling breadcrumbs.
...
We'll use the Router events, which conveniently provides an observable that we can subscribe to and listen for route changes, react and create a set of breadcrumbs and the title.
